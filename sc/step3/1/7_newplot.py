...
def load_case_map(case_map_path):
    ...
    internally for matching. Identical to sc/step1/step1/9_plot.py's helper of
    the same name."""
...
def parse_cluster_txt(cluster_txt_path):
    ...
    # sc/step3/2/7_newplot.py の同名関数と揃えてある: DEL/INS側の nested.txt 由来
    # cluster_txt は "# cluster 6-2" のような "{top}-{sub}" 複合番号ヘッダを持ちうる
    # ため、\S+ で番号トークン全体を1つのIDとして捉える（\d+ だと先頭の数字しか
    # 拾えず、"6-1"/"6-2" が同じクラスタに潰れてしまう）。SNP側は通常 "# cluster N"
    # の単純な形式のみなので、この変更による見た目の変化はない。
...
def parse_new_cluster_txt(path):
    ...
    戻り値: {sample_name: label_str}
      label_str は "cluster 1" のような文字列 または "H22" のようなハプロタイプ名
    """
    result = {}
    current_label = None
...
def main(old_input_path, new_input_path, pca_model_path, cluster_txt_path, output_file, plot_pc,
    ...
    old_feature_cols = [c for c in old_df.columns if c != old_sample_col]
    X_old = old_df[old_feature_cols].values
    reduced_old = pca.transform(X_old)

    # 4) 新データ（旧データの特徴量列に揃えてから transform）
    new_df = pd.read_csv(new_input_path, sep="\t")
    new_sample_col = detect_sample_col(new_df)
    X_new = new_df.reindex(columns=old_feature_cols, fill_value=0).values
    ...
    # 新サンプルは、配属されたクラスタと同じ色（四角マーカーの色分けはmain()側で行う）。
    ...
    fig.legend(handles=patches, title="Clusters / Alleles",
               loc='center left', bbox_to_anchor=(1.03, 0.5))

    # 凡例を含めて保存（bbox_inches='tight' が凡例分を自動で含める）
