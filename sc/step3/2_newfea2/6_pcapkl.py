...
def save_pca_model(input_tsv: str, output_pca_model: str, n_components: int):
    ...
    # 特徴量0列（学習データ全予測がNONE）はPCA不可 → nullモデルを保存して正常終了
    if n_features == 0:
        print("[WARN] 0 feature columns (all predictions in the training data are NONE). Skipping PCA.")
        joblib.dump({"null_model": True, "n_components": 0}, output_pca_model)
        print(f"[OK] Saved null model: {output_pca_model}")
        return
