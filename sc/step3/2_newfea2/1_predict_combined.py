...
DELモデルはDEL特徴量系、INSモデルはINS特徴量系を用いて予測する。
...
# INS_notTP のみ、通常の閾値（threshold、既定0.98）とは別に専用の閾値を使う。
# INS_TP / DEL は引き続き threshold をそのまま使う。
    ...
    for inp in inputs:
        ...
        if p.is_dir():
            ...
            if str(p).endswith(".bam"):
                ...
                print(f"[WARN] Not a .bam extension (skipping): {inp}")
        else:
...
def load_targets_tsv(tsv_path: str) -> pd.DataFrame:
    ...
    if df.empty:
        print(f"[WARN] Targets TSV has no data rows (header only): {tsv_path}")
        return pd.DataFrame({"CHROM_RAW": pd.Series(dtype="object"),
...
def _maybe_add_prob_ins(row: Dict[str, Any], classes: List[str], probs) -> None:
    ...
    if "INS" in cls_set:
        try:
            ...
            pass
        return
    if "INS_short" in cls_set or "INS_long" in cls_set:
        p = sum(float(pr) for c, pr in zip(classes, probs) if c in ("INS_short", "INS_long"))
...
def _process_chunk_del(args: Dict[str, Any]) -> Dict[str, List]:
    ...
    model_del_path  = args["model_del_path"]

    model_del = joblib.load(model_del_path)
    del_classes = list(model_del.classes_)
    ...
    for pos0 in positions0:
        ...
        results_features.append(feat_row)
        results_raw.extend(_build_raw_rows_del(chrom, pos1, pos_counts))
    ...
    return {"del": results_del, "features": results_features, "raw": results_raw}


def _process_chunk_ins(args: Dict[str, Any]) -> Dict[str, List]:
    bam_path        = args["bam_path"]
    chrom           = args["chrom"]
    positions0      = args["positions0"]
    threshold       = args["threshold"]
    emit_n          = args.get("emit_n_for_targets", False)
    mean_depth      = args.get("mean_depth", float("nan"))
    model_ins_path  = args["model_ins_path"]
    ins_mq          = args.get("ins_mq", INS_MQ_DEFAULT)
    total_chrom_count = args.get("total_chrom_count")
    median_isize    = args.get("median_isize", float("nan"))

    model_ins = joblib.load(model_ins_path)
    ins_classes = list(model_ins.classes_)
    ...
    for pos0 in positions0:
        pos1 = pos0 + 1
        feats, pos_counts = _features_ins(bam_path, chrom, pos1, mean_depth, ins_mq,
                                          total_chrom_count, median_isize)

        feat_row = {"CHROM": chrom, "POS": pos1}
        feat_row.update(feats)
        results_features.append(feat_row)
        results_raw.extend(_build_raw_rows_ins(chrom, pos1, pos_counts))
        ...
        prob  = float(probs[idx])
        threshold_eff = INS_NOTTP_THRESHOLD if label == "INS_notTP" else threshold

        if prob >= threshold_eff:
            row = {"CHROM": chrom, "POS": pos1, "TYPE": label, "PROB": round(prob, 4)}
    ...
    return {"ins": results_ins, "features": results_features, "raw": results_raw}


# ============================================================
# 1BAMの予測実行（DEL / INS 独立）
# ============================================================
...
def _run_prediction(
    ...
    n_jobs: int,
    out_pred: str,
    out_feat: Optional[str],
    out_raw: Optional[str],
    ins_mq: int = INS_MQ_DEFAULT,
    total_chrom_count: Optional[int] = None,
    median_isize: float = float("nan"),
    used_targets: bool = True,
):
    """DELまたはINSの予測を実行してTSVを書き出す。

    used_targets: positions_by_chrom が --targets 由来か --region（領域全体
    スキャン）由来かを表す。どちらの場合も positions_by_chrom 経由で処理される
    ため、ログメッセージの文言を正しく出し分けるためだけに使う（実際の処理は
    変わらない）。
    """
    is_del = (mode == "del")
    classes = list(model.classes_)
    feature_cols = DEL_FEATURE_COLS if is_del else INS_FEATURE_COLS
    chunk_fn = _process_chunk_del if is_del else _process_chunk_ins
    feat_output_order = ["CHROM", "POS"] + feature_cols

    print(f"  [{mode.upper()}] classes: {classes}")
    print(f"  [{mode.upper()}] features: {feature_cols}")
    ...
    def _run_at_pos(chrom: str, pos0: int) -> None:
        ...
        feat_row = {"CHROM": chrom, "POS": pos1}
        feat_row.update(feats)
        results_feat.append(feat_row)

        X = _align_features(pd.DataFrame([feats]), model, feature_cols)
        probs = model.predict_proba(X)[0]
        idx   = probs.argmax()
        label = str(classes[idx])
        prob  = float(probs[idx])
        threshold_eff = INS_NOTTP_THRESHOLD if (not is_del and label == "INS_notTP") else threshold

        if prob >= threshold_eff:
            row = {"CHROM": chrom, "POS": pos1, "TYPE": label, "PROB": round(prob, 4)}
            for cls, p in zip(classes, probs):
                row[f"PROB_{cls}"] = round(float(p), 4)
            results_pred.append(row)
        elif emit_n:
            row = {"CHROM": chrom, "POS": pos1, "TYPE": "N", "PROB": round(prob, 4)}
            for cls, p in zip(classes, probs):
                row[f"PROB_{cls}"] = round(float(p), 4)
    ...
    if positions_by_chrom is not None:
        scan_desc = "Targets" if used_targets else "Region"
        if n_jobs == 1:
            for chrom in sorted(positions_by_chrom.keys()):
                pos_list0 = positions_by_chrom[chrom]
                for pos0 in tqdm(pos_list0, desc=f"  [{mode.upper()}] {scan_desc} {chrom} ({len(pos_list0)} pos)"):
                    _run_at_pos(chrom, pos0)
        else:
            model_key = "model_del_path" if is_del else "model_ins_path"
            tasks = [
                {"bam_path": bam_path, "chrom": chrom, "positions0": pos_list0,
                 "threshold": threshold,
                 model_key: model_path,
                 "emit_n_for_targets": True, "mean_depth": mean_depth,
                 "ins_mq": ins_mq, "total_chrom_count": total_chrom_count,
                 "median_isize": median_isize}
                for chrom, pos_list0 in positions_by_chrom.items()
            ]
            mode_label = "targets mode" if used_targets else "full region scan"
            print(f"  [{mode.upper()}] running {len(tasks)} task(s) in parallel across {n_jobs} process(es) ({mode_label})...")
            with ProcessPoolExecutor(max_workers=n_jobs) as executor:
                futures = {executor.submit(chunk_fn, t): t for t in tasks}
                for future in tqdm(as_completed(futures), total=len(futures),
                                   desc=f"  [{mode.upper()}] Processing"):
                    try:
                        result = future.result()
                        results_pred.extend(result[mode])
                        results_feat.extend(result["features"])
                        results_raw.extend(result["raw"])
                    except Exception as e:
                        print(f"[ERROR] {e}")
    else:
        if n_jobs == 1:
            for chrom, r_start, r_end in spans_scan:
                for pos0 in tqdm(range(r_start, r_end, step), desc=f"  [{mode.upper()}] Scanning {chrom}"):
                    _run_at_pos(chrom, pos0)
        else:
            model_key = "model_del_path" if is_del else "model_ins_path"
            tasks = [
                {"bam_path": bam_path, "chrom": chrom,
                 "positions0": list(range(r_start, r_end, step)),
                 "threshold": threshold,
                 model_key: model_path,
                 "emit_n_for_targets": False, "mean_depth": mean_depth,
                 "ins_mq": ins_mq, "total_chrom_count": total_chrom_count,
                 "median_isize": median_isize}
                for chrom, r_start, r_end in spans_scan
            ]
            print(f"  [{mode.upper()}] running {len(tasks)} task(s) in parallel across {n_jobs} process(es) (full scan)...")
            with ProcessPoolExecutor(max_workers=n_jobs) as executor:
                futures = {executor.submit(chunk_fn, t): t for t in tasks}
                for future in tqdm(as_completed(futures), total=len(futures),
                                   desc=f"  [{mode.upper()}] Processing"):
    ...
    if out_feat:
        ...
        if not df_feat.empty:
            ...
            df_feat = df_feat[[c for c in feat_output_order if c in df_feat.columns]]
        df_feat.to_csv(out_feat, sep="\t", index=False)
        print(f"  ✓ Saved [{mode.upper()}] features: {out_feat} ({len(df_feat)} positions)")

    if out_raw:
        raw_cols = DEL_RAW_COLS if is_del else INS_RAW_COLS
        df_raw = pd.DataFrame(results_raw, columns=raw_cols)
        if not df_raw.empty:
            df_raw = df_raw.sort_values(by=["CHROM", "BASE_POS", "WIN_POS"]).reset_index(drop=True)
        df_raw.to_csv(out_raw, sep="\t", index=False)
        print(f"  ✓ Saved [{mode.upper()}] raw: {out_raw} ({len(df_raw)} rows)")

    return results_feat


def _all_zero_depth(results_feat: list, mean_depth: float) -> bool:
    """True if every scanned position's raw read depth (DEPTH_RATIO's
    numerator, before dividing by mean_depth) was exactly 0 -- i.e. this
    sample's BAM has no reads anywhere across the scanned positions, a strong
    signal the gene is absent/deleted (not just "insufficient data").

    Only meaningful when mean_depth is itself a valid, non-zero value: with a
    valid non-zero denominator, DEPTH_RATIO == 0.0 unambiguously means the raw
    pileup count was 0 (0/x == 0 only when the numerator is 0). If mean_depth
    is NaN/0, DEPTH_RATIO's own _nan_to_zero() clamp would make every position
    look like "0" regardless of actual coverage, so this must return False in
    that case rather than risk a false positive."""
    if not results_feat or math.isnan(mean_depth) or mean_depth <= 0:
        return False
    return all(row.get("DEPTH_RATIO", 0.0) == 0.0 for row in results_feat)
...
def predict_combined(
    ...
    if precomputed_mean_depth is not None and not math.isnan(precomputed_mean_depth):
        # 事前計算済みの値（--mean-depth-tsv）を使う。領域抽出済みBAM（±10kb等）を
        # --bam に渡す場合、このBAM自身から再計算すると total_mapped が激減する一方で
        # genome_length はヘッダーの宣言値のまま変わらないため、mean_depthが実際より
        # 大幅に小さく見積もられ、DEPTH_RATIO系の特徴量が全て壊れる。必ず全長BAMに
        # 対して estimate_mean_depth.py で事前計算した値をここで再利用する。
        mean_depth = precomputed_mean_depth
        print(f"  [INFO] Mean depth (using precomputed value): {mean_depth:.2f}x")
    else:
        print("  [INFO] Estimating mean depth...")
    ...
    if precomputed_median_isize is not None and not math.isnan(precomputed_median_isize):
        ...
        print("  [INFO] Estimating library median insert size...")
        median_isize = estimate_median_insert_size(bam_path)
        if not math.isnan(median_isize):
            print(f"  [INFO] Estimated median insert size: {median_isize:.1f}")
        else:
            print("  [INFO] Estimated median insert size: NaN")

    bam = pysam.AlignmentFile(bam_path, "rb")
    alias = build_bam_chrom_alias(list(bam.references))
    print(f"  [INFO] BAM references example: {list(bam.references)[:5]} (n={len(bam.references)})")

    # MATE_DIFFCHR_DIVERSITY_RATIO の分母（BAMヘッダの@SQ行数）。1_bam_choice.py の
    # 領域抽出は元BAMのヘッダをそのままコピーして書き出すため、±10kb抽出済みBAMでも
    # この値は元の全長BAMと一致する（sc/step1/step2_newfea2 と同じ理由で precompute 不要）。
    ...
    if targets_tsv:
        ...
        if total == 0:
            print("  [WARN] No valid target positions (0). "
                  "Writing an empty prediction TSV and skipping.")
            bam.close()
            for path in (out_del, out_ins):
                if path:
                    pd.DataFrame(columns=["CHROM", "POS", "TYPE", "PROB"]).to_csv(
                        path, sep="\t", index=False)
            return
    elif mapped_region:
        # --targets なし・--region あり → 遺伝子領域全体をスキャン（emit_n=True）
        r_chrom, r_start, r_end = parse_region(mapped_region)
        if r_start is not None and r_end is not None and r_chrom in bam.references:
            chrom_len = bam.get_reference_length(r_chrom)
            pos0_list = [p for p in range(r_start - 1, r_end, step) if 0 <= p < chrom_len]
            if pos0_list:
                positions_by_chrom = {r_chrom: pos0_list}
                total = len(pos0_list)
                print(f"  [INFO] region {mapped_region}: scanning all {total} positions (step={step})")
            else:
                print(f"  [WARN] region {mapped_region}: no valid positions. Skipping.")
                bam.close()
                return
        else:
            spans_scan = build_spans_for_scan(bam, mapped_chroms, mapped_region)
            if not spans_scan:
                print("  [WARN] No span to scan. Skipping.")
                bam.close()
                return
    else:
        spans_scan = build_spans_for_scan(bam, mapped_chroms, None)
        if not spans_scan:
            print("  [WARN] No span to scan. Skipping.")
            bam.close()
            return
    ...
    used_targets = bool(targets_tsv)

    # 「遺伝子欠失」判定: このBAMで実際に実行したSV種別（DEL/INS）の走査
    # 座位すべてで生リード深度が0だったかどうか。実行した種別すべてで
    # 0だった場合のみ True（--model-del/--model-insどちらか一方のみ実行時は
    # そちらだけで判定。両方実行時は両方が0であることを要求）。
    ran_any = False
    all_zero_so_far = True

    # DEL予測
    if model_del is not None and out_del is not None:
        del_feat = _run_prediction(
            bam_path, "del", model_del, model_del_path,
            positions_by_chrom, spans_scan,
            mean_depth, step, threshold, n_jobs,
            out_del, out_del_feat, out_del_raw,
            median_isize=median_isize,
            used_targets=used_targets,
        )
        ran_any = True
        all_zero_so_far = all_zero_so_far and _all_zero_depth(del_feat, mean_depth)

    # INS予測
    if model_ins is not None and out_ins is not None:
        ins_feat = _run_prediction(
            bam_path, "ins", model_ins, model_ins_path,
            positions_by_chrom, spans_scan,
            mean_depth, step, threshold, n_jobs,
            out_ins, out_ins_feat, out_ins_raw,
            ins_mq=ins_mq, total_chrom_count=total_chrom_count,
            median_isize=median_isize,
            used_targets=used_targets,
        )
        ran_any = True
        all_zero_so_far = all_zero_so_far and _all_zero_depth(ins_feat, mean_depth)

    return ran_any and all_zero_so_far
...
def main():
    ...
    parser.add_argument("--step", type=int, default=1,
                        help="スキャンステップ（full scanモード、デフォルト: 50）")
    parser.add_argument("--threshold", type=float, default=0.98,
                        help="予測確率の閾値（デフォルト: 0.6）")
    ...
    parser.add_argument("--mean-depth-tsv", default=None,
                        help="estimate_mean_depth.py が出力したTSV（sample, mean_depth, "
                             "median_isize）。指定時はサンプルごとにこの値を使い、BAMからの"
                             "再計算を行わない。領域抽出済みBAM（±10kb等）を --bam に渡す場合は"
                             "必須（抽出前の全長BAMに対して estimate_mean_depth.py を実行した"
                             "結果を渡すこと）。")

    args = parser.parse_args()

    if args.model_del is None and args.model_ins is None:
        parser.error("At least one of --model-del or --model-ins must be specified.")
    ...
    # DEL/INSは別々の起動（___run_pipeline.pyがこのスクリプトを--model-del/
    # --model-insそれぞれで別プロセスとして呼ぶ）だが、同じoutdirを共有する。
    # 先に実行された側（通常はDEL）が既にGENE_ABSENT判定済みのサンプルは、
    # 全く同じ結論になるはずのDEPTH_RATIO計算をもう一方の側でも繰り返さない
    # よう、ここで読み込んで予測自体をスキップする。
    gene_absent_path = Path(args.outdir) / "gene_absent_samples.txt"
    known_gene_absent = set()
    if gene_absent_path.is_file():
        with open(gene_absent_path, encoding="utf-8") as f:
            known_gene_absent = {line.strip().upper() for line in f if line.strip()}
        if known_gene_absent:
            print(f"[INFO] Already determined GENE_ABSENT: {len(known_gene_absent)} sample(s) "
                  f"(from the earlier DEL/INS prediction, {gene_absent_path})")

    gene_absent_samples = set(known_gene_absent)
    ...
    for bam in bam_files:
        ...
        if sample.upper() in known_gene_absent:
            print(f"• Skipping (already determined GENE_ABSENT): {sample}")
            for path in (out_del, out_ins):
                if path:
                    pd.DataFrame(columns=["CHROM", "POS", "TYPE", "PROB"]).to_csv(
                        path, sep="\t", index=False)
        ...
        if args.skip_existing:
            ...
            if len(existing) == expected:
                print(f"• Skipping (already exists): {sample}")
                continue
        ...
        gene_absent = predict_combined(
            bam, model_del, model_ins,
            args.model_del, args.model_ins,
            out_del, out_ins,
        ...
        if gene_absent:
            print(f"  [INFO] {sample}: raw read depth is 0 at every scanned position -> recording as a GENE_ABSENT candidate")
            gene_absent_samples.add(sample.upper())

    with open(gene_absent_path, "w", encoding="utf-8") as f:
        for s in sorted(gene_absent_samples):
            f.write(f"{s}\n")
    if gene_absent_samples:
        print(f"[OK] Wrote {len(gene_absent_samples)} GENE_ABSENT candidate(s): {gene_absent_path}")
