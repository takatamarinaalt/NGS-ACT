...
def get_cluster_color(cluster_id):
    ...
    結合後（stage2）は flat cluster 番号が10を超えることがあり、固定10色だけの
    辞書だと未定義のクラスタが NaN になって ax.scatter が例外を投げていた。"""
    fixed = {
...
def main(input_path, cluster_path, output_file, n_components, plot_pc, variance_output, pca_model_out=None, case_map=None):
    ...
    # 6. 色の割り当て
    colors = merged_df["cluster"].map(get_cluster_color)

    # 7. 凡例パッチ（任意ラベル対応）
    valid_clusters = sorted(merged_df["cluster"].dropna().unique())
    patches = [
        mpatches.Patch(color=get_cluster_color(c), label=cluster_labels.get(c, f"Cluster {c}"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pca-model-out", default=None, help="PCAモデルをjoblib形式で保存するパス（step10・step3で再利用）")
