...
def convert(gene_id, coord_type, pos, gene_data):
    ...
    if not segments:
        result["message"] = "No CDS/exon information in GFF"
        return result
    ...
    with open(tsv_path) as f:
        ...
        for line in f:
            ...
            try:
                ...
                print(f"[WARN] Skipped (parse error): {line} → {e}", file=sys.stderr)
                continue
...
def main():
    ...
    print(f"  → Loaded {len(gene_data)} genes", file=sys.stderr)
    ...
    if not args.gene or not args.coord_type or args.pos is None:
        parser.error("--gene / --coord-type / --pos are required for single conversion (use --batch for batch processing)")

    # 遺伝子名（Hd1等）でも遺伝子IDでも受け付ける
    gene_id = resolve_gene_id(args.gene, gene_data, name_to_id)
    if gene_id != args.gene:
        print(f"  Gene name '{args.gene}' resolved to {gene_id}", file=sys.stderr)
    ...
    if result["status"] == "OK":
        ...
        if result["cds_pos"]:
            ...
            print(f"    Genome position: {result['chrom']}:{result['genome_pos']}")
